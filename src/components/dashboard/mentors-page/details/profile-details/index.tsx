...
const PersonalInfo = ({ user }: { user: AllMentorsType | undefined }) => {
  if (!user) return null;
  return (
    <CardInfo
        ...
        <div className='grid grid-cols-1 md:grid-cols-3 xl:grid-cols-5 gap-10'>
          ...
          <div className='flex flex-col gap-5'>
            <ProfileInfo
              ...
              value={user.expertise_areas?.map((item) => item.skill)?.join(', ')}
            />
          ...
          <div className='flex flex-col gap-5'>
            <ProfileInfo
              label='Track'
              value={<span className='capitalize'>{user.track}</span>}
            />
