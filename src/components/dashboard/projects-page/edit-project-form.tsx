...
const EditProjectForm = ({
  ...
  const formik = useFormik({
    initialValues: {
      ...
      difficultyLevelBadge: project?.difficulty_level_badge ?? '',
      technologyTags: project?.technology_tags ?? '',
      longDescription: project?.long_description ?? '',
      maximumXP: project?.maximum_xp_assignable ?? '',
  ...
  const submitValues = async () => {
    try {
      setLoading(true);
      await appAxios.put(`/projects/admins/${project?.id}`, {
  ...
  return (
    <CustomCard
      title='Overview'
      controls={
        <div className='px-5 py-3 flex items-center justify-between w-full border-t-[#F0F2F5] border-t-[2px]'>
          <Button
            onClick={closeModal}
            disabled={loading}
      ...
      <div className='w-full'>
        ...
        <Dropdown
          ...
          value={{
            ...
            label:
              courses?.find((course) => course.id === formik.values.courseId)?.title ??
              '',
          }}
          formik={formik}
        ...
        <Dropdown
          ...
          value={{
            ...
            label:
              learningPaths?.find((path) => path.id === formik.values.learningPathId)
                ?.title ?? '',
          }}
          name='learningPathId'
          formik={formik}
