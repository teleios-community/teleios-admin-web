...
interface Props {
  closeModal: () => void;
  selected: ProjectSubmissionType | undefined;
  open: boolean;
}

function SubmissionDetailsModal({ closeModal, open, selected }: Props) {
  ...
  return (
    <CustomModal
      ...
      width='608px'
    >
      {(Object.keys(selected) as Array<keyof ProjectSubmissionType>).map((key) => (
