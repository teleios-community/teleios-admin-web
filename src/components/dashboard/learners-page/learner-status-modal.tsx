...
function LearnerStatusModal({ closeModal, reload, open, selected }: Props) {
  ...
  const executeDelete = async () => {
    try {
      setLoading(true);
      await appAxios.put(`/learners/admin/specific-user/${selected?.id}`, {
        status: isActive ? 'inactive' : 'active',
      });
      closeModal();
      reload();
      sendFeedback(`User ${isActive ? 'deactivated' : 'activated'}`, 'success');
    } catch (error) {
